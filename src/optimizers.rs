...
impl Adam {
	...
	let mut a = Adam {
	    alpha, beta1: 0.0, beta2: 0.0, start_beta1, start_beta2,
	    old_m, old_v,
	};
...
impl Optimizer for Adam {
	...
	for layer_n in 0..gradient.len() {
		...
		for weight_n in 0..gradient[layer_n][node_n].len() {
		    temp_m = b1 * self.old_m[layer_n][node_n][weight_n] + (1f64 - b1) * gradient[layer_n][node_n][weight_n];
		    temp_v = b2 * self.old_v[layer_n][node_n][weight_n] + (1f64 - b2) * gradient[layer_n][node_n][weight_n].powi(2);
		    mhat = temp_m / (1f64 - b1);
		    vhat = temp_v / (1f64 - b2);
